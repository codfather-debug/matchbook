    ...
    <div className={`rounded-2xl border p-4 space-y-3 transition-all ${
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className={`text-sm font-black ${a.unlocked ? "text-gray-900" : "text-gray-500"}`}>{a.title}</p>
            <p className="text-xs text-gray-400 mt-0.5 leading-snug">{a.description}</p>
          </div>
        </div>
        {a.unlocked && (
          <span className="shrink-0 text-[10px] font-black text-lime-700 bg-lime-50 px-2 py-0.5 rounded-full uppercase tracking-wide">
            Unlocked
          </span>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] text-gray-400">{a.progressText}</span>
          {a.unlockedAt && (
    ...
    <main className="min-h-screen bg-white max-w-sm mx-auto pb-24">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-gray-400 text-xs font-bold tracking-widest uppercase">Achievements</p>
        </div>
        <h1 className="text-2xl font-black text-gray-900">Badges & Progress</h1>
        <div className="flex items-center gap-2 mt-1">
          <p className="text-gray-500 text-sm">{unlocked} / {achievements.length} unlocked</p>
          {unlocked > 0 && (
