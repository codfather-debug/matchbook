  ...
  return (
    <div className="relative w-24 h-24 flex items-center justify-center">
      ...
      <div className="text-center z-10">
        ...
        <p className="text-[10px] text-white/30 font-bold">WIN</p>
      </div>
    </div>
  );
}
    ...
    <main className="min-h-screen bg-[#1e1e2a] max-w-sm mx-auto pb-10">
      ...
      <div className="sticky top-0 z-10 bg-[#1e1e2a]/90 backdrop-blur-xl border-b border-white/[0.06] px-5">
        <div className="flex items-center justify-between h-14">
          ...
          <span className="text-xs font-bold tracking-[0.2em] uppercase text-white/30">Opponent</span>
          <div className="w-12" />
        </div>
      </div>
      ...
      <div className="px-5 pt-6 pb-10 space-y-6">
        {matches.length === 0 ? (
          <div className="text-center py-16 space-y-2">
            <p className="text-white/30">No matches found for {decodedName}.</p>
            <Link href="/history" className="text-lime-400 text-sm font-semibold">← Back to history</Link>
          </div>
        ) : (
          <>
            {/* Hero */}
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 flex items-center gap-5">
              {/* Avatar */}
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center flex-shrink-0">
                <span className="text-xl font-black text-white/60">{initials}</span>
              </div>
              {/* Name + record */}
              <div className="flex-1 min-w-0">
                <p className="text-xs font-black tracking-widest uppercase text-white/30 mb-0.5">vs</p>
                <h1 className="text-xl font-black text-white truncate">{decodedName}</h1>
                <p className="text-sm text-white/40 mt-0.5">{record.wins}W – {record.losses}L · {matches.length} match{matches.length !== 1 ? "es" : ""}</p>
              </div>
              {/* Win rate arc */}
              ...
              <div className="grid grid-cols-2 gap-2">
                {mentalAvg !== null && (
                  <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-3 text-center">
                    <p className="text-xs text-white/30 font-bold uppercase tracking-widest">Mental Avg</p>
                    <p className={`text-2xl font-black mt-0.5 ${mentalAvg >= 7 ? "text-lime-400" : mentalAvg >= 5 ? "text-amber-400" : "text-red-400"}`}>
                      {mentalAvg}
                    </p>
                  </div>
                )}
                {execAvg !== null && (
                  <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-3 text-center">
                    <p className="text-xs text-white/30 font-bold uppercase tracking-widest">Exec Avg</p>
                    <p className={`text-2xl font-black mt-0.5 ${execAvg >= 7 ? "text-lime-400" : execAvg >= 5 ? "text-amber-400" : "text-red-400"}`}>
                      {execAvg}
                    </p>
                  </div>
                )}
            ...
            {Object.keys(surfRates).length > 0 && (
              <section className="space-y-2">
                <p className="text-xs font-black tracking-widest uppercase text-white/30">By Surface</p>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className={`text-sm font-black px-2.5 py-0.5 rounded-full ${win ? "bg-lime-400/15 text-lime-400" : "bg-red-500/15 text-red-400"}`}>
                            {win ? "W" : "L"}
                          </span>
                        ...
                        {m.scouting?.keyToWin && (
                          <p className="text-xs text-white/30 italic mt-0.5 truncate">🔑 {m.scouting.keyToWin}</p>
                        )}
                      </div>
